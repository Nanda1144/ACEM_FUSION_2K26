...
import { eventsApi, footerSettingsApi } from '@/db/api';
...
export default function EventDetailPage() {
  ...
  const loadFooterColor = async () => {
    try {
      ...
      setFooterBgColor(data?.bg_color || null);
    } catch (error) {
      console.error('Error loading footer color:', error);
    }
  };

  const textPanelStyle = {
    backgroundColor: footerBgColor || 'rgba(0,0,128,0.75)',
  };
          ...
          <div className="container mx-auto max-w-6xl">
            ...
            >
              <div className="grid grid-cols-1 lg:grid-cols-2 gap-8 mb-8">
                ...
                <div className="rounded-2xl p-6 text-white" style={textPanelStyle}>
                  <h1 className="text-4xl md:text-5xl font-bold mb-4 gradient-text">{event.name}</h1>

                  {event.description_format === 'list' ? (
                    <ul className="text-lg text-white mb-6 space-y-2 list-disc list-inside">
                      {event.description
                        .split('\n')
                    ...
                    <div
                      className="text-lg text-white mb-6 text-justify leading-relaxed"
              ...
              <div className="grid grid-cols-1 lg:grid-cols-2 gap-8 mb-8">
                {event.rules && (
                  <Card className="text-white border-white/20" style={textPanelStyle}>
                    <CardContent className="pt-6">
                      ...
                      <div className="max-w-none text-white">
                        <p className="whitespace-pre-wrap text-white">{event.rules}</p>
                      </div>
                    </CardContent>
                  </Card>
                )}
                ...
                {event.instructions && (
                  <Card className="text-white border-white/20" style={textPanelStyle}>
                    <CardContent className="pt-6">
                      ...
                      <div className="max-w-none text-white">
                        <p className="whitespace-pre-wrap text-white">{event.instructions}</p>
                      </div>
                    </CardContent>
                  </Card>
                )}
                    ...
                    <CardContent className="pt-6">
                      ...
                      <div className="space-y-4">
                        {event.staff_coordinators.map((coordinator, index) => (
                          <div key={index} className="p-4 rounded-lg border border-white/20" style={textPanelStyle}>
                            <h3 className="font-semibold text-lg mb-2">{coordinator.name}</h3>
                            <div className="flex items-center gap-2 text-sm text-white/90">
                              <Phone className="h-4 w-4" />
                              <a href={`tel:${coordinator.contact}`} className="hover:text-white transition-colors">
                                {coordinator.contact}
                              </a>
                            </div>
                          </div>
